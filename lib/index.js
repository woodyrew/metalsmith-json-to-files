...
var options_schema = joi.object().keys({
    source_path         : joi.string()
  , use_metadata        : joi.boolean()
  , properties_to_remove: joi.array()
}).xor('source_path', 'use_metadata');

var metadata_schema = joi.object().keys({
    source_file     : joi.string()
  , metadata_key    : joi.string()
  , filename_pattern: joi.string().required()
  , as_permalink    : joi.boolean()
}).xor('source_file', 'metadata_key');
 ...
 * @param  {String} options.source_path Path for source JSON files
 * @param  {Boolean} options.use_metadata If true, use metadata values as JSON source
 *
 * Either options.source_path or options.use_metadata must be present.
 *
    ...
    return function (files, metalsmith, done) {
        ...
        var process_file = function process_file (file) {
            ...
            if (options.source_path) {
                ...
                json = jsonfile.readFileSync(source_filepath);
            }
            else if (options.use_metadata) {
                var metadata = metalsmith.metadata();
                json = metadata[file_meta.json_files.metadata_key];
            }
